document.addEventListener('DOMContentLoaded', () => {
    ...
    let santaPollInterval = null;

    const santaCreateBtn = document.getElementById('santa-create-btn');
    const santaJoinBtn = document.getElementById('santa-join-btn');
    const santaJoinInput = document.getElementById('santa-join-input');
    const santaStartBtn = document.getElementById('santa-start-btn');
    ...
    const targetWishlistBtn = document.getElementById('target-wishlist-btn');

    // Deep Link Check for Room
    if (window.location.search.includes('santa_room=')) {
        const params = new URLSearchParams(window.location.search);
        const room = params.get('santa_room');
        if (room) {
            currentRoomId = room;
            joinSantaRoom(room);
        }
    ...
    if (santaCreateBtn) {
        santaCreateBtn.addEventListener('click', async () => {
            santaCreateBtn.disabled = true;
            santaCreateBtn.innerText = "Создаем...";

            const res = await apiFetch('/santa', {
                method: 'POST',
                body: JSON.stringify({
                    action: 'create',
                    user: {
            ...
            santaCreateBtn.disabled = false;
            santaCreateBtn.innerText = "Создать комнату";
        });
    }

    if (santaJoinBtn) {
        santaJoinBtn.addEventListener('click', () => {
            const code = santaJoinInput.value.trim();
            if (code.length < 5) {
    ...
    async function joinSantaRoom(id) {
        currentRoomId = id;
        const res = await apiFetch('/santa', {
            method: 'POST',
            body: JSON.stringify({
        ...
        if (res && res.success) {
            enterLobby(res.room);
            document.querySelector('[data-target="santa-view"]').click();
        } else {
    ...
    function enterLobby(room) {
        // Switch UI
        document.getElementById('santa-home-state').classList.add('hidden');
        document.getElementById('santa-lobby-state').classList.remove('hidden');
        document.getElementById('santa-game-state').classList.add('hidden');

        lobbyCodeDisplay.innerText = currentRoomId;
        document.getElementById('lobby-room-name').innerText = room.roomName;
        ...
        if (room.status === 'started') {
            ...
            // Polling
            if (!santaPollInterval) {
                santaPollInterval = setInterval(pollRoom, 3000);
            }

            // Admin check
            const isAdmin = room.adminId == (userProfile.id || userProfile.username);
    ...
    function renderParticipants(list) {
        const container = document.getElementById('santa-participants-list');
        container.innerHTML = '';
        list.forEach(p => {
            const div = document.createElement('div');
            div.className = 'participant-item';
            div.innerHTML = `<img src="${p.avatar}" class="participant-avatar"><span>${p.name}</span>`;
            container.appendChild(div);
        });
    }

    if (santaStartBtn) {
        santaStartBtn.addEventListener('click', async () => {
            const res = await apiFetch('/santa', {
                method: 'POST',
                body: JSON.stringify({
                    action: 'start',
                    roomId: currentRoomId,
                    user: { id: userProfile.id || userProfile.username }
                })
            });
    ...
    function enterGame(room) {
        clearInterval(santaPollInterval);
        santaPollInterval = null;

        document.getElementById('santa-lobby-state').classList.add('hidden');
        document.getElementById('santa-game-state').classList.remove('hidden');

        // Find my target
        const myId = userProfile.id || userProfile.username;
        const me = room.participants.find(p => p.id == myId);

        if (me && me.targetId) {
            const target = room.participants.find(p => p.id == me.targetId);
            if (target) {
                setupReveal(target);
            }
        }
    ...
    function setupReveal(target) {
        ...
        const imgEl = document.getElementById('target-avatar');

        nameEl.innerText = target.name;
        imgEl.src = target.avatar;

        targetWishlistBtn.onclick = () => {
            if (target.id && String(target.id).includes('-')) {
                loadSharedProfile(target.id);
            } else {
                alert("Профиль этого пользователя пока недоступен :(");
            }
        };
    }
    ...
    if (revealCover) {
        revealCover.addEventListener('click', () => {
            revealCover.style.transform = "scale(0) rotate(180deg)";
    ...
    if (copyLobbyCodeBtn) {
        copyLobbyCodeBtn.addEventListener('click', () => {
            ...
            alert("Код скопирован: " + currentRoomId);
        });
    }
